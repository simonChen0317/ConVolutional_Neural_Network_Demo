...
def evaluate(mnist):
    with tf.Graph().as_default() as g:
        ...
        x = tf.placeholder(tf.float32, [mnist.validation.num_examples,  # 第一维表示一个batch中样例的个数。
                                        mnist_inference.IMAGE_SIZE,  # 第二维表示图片的尺寸。
                                        mnist_inference.IMAGE_SIZE,
                                        mnist_inference.NUM_CHANNELS],  # 第四维表示图片的深度，对于RBG格式的图片，深度为5
                           name='x-input')
        y_ = tf.placeholder(tf.float32, [None,mnist_inference.OUTPUT_NODE], name='y-input')
        xi = mnist.validation.images
        reshaped_xs =np.reshape(mnist.validation.images, (mnist.validation.num_examples, mnist_inference.IMAGE_SIZE, mnist_inference.IMAGE_SIZE, mnist_inference.NUM_CHANNELS))
